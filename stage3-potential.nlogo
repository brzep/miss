...
to setup-exit [x y]
  ask patch x y [
    set pcolor grey
    set exit? true
    spread-base-potential 99
  ]

  ; diffuse potential 0.99 ; works in a different way
end

to spread-base-potential [p]
  if pcolor = grey and base-potential < p [
...
to uphill-if-empty
  ...
  let p max-one-of neighbors with [ count turtles-here = 0 ] [potential]

  if p != nobody and [potential] of p > potential [
    face p
    move-to p
  ]
end
